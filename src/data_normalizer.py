...
class DataNormalizer:
	...
	def _clean_city(self, value: Any) -> str:
		"""Reduce a location-like value to a single city name in Title Case.
		- Take content before first '|'
		- If contains ' in ', keep the part after it
		- If contains ',', keep the part before it
		...
		if len(parts) == 2:
			text = parts[1].strip()
		# If a state or country is present, keep only the city
		text = text.split(',')[0].strip()
		text = re.sub(r"\s+", " ", text)
		return text.title() if text else 'NA'
	...
	def _normalize_lead(self, lead: Dict[str, Any]) -> Dict[str, Any]:
		...
		# Enforce city-only Title Case for Location
		normalized['Location'] = self._clean_city(loc)
